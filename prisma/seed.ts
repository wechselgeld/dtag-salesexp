...
async function main() {
    ...
    const mobilL = await prisma.product.create({
        data: {
            ...
            allowPlanChange: true,
            activationFeeNew: 39.95,
            features: JSON.stringify(['5G', 'Phone Flat', 'SMS Flat', 'EU Roaming', 'StreamOn']),
        }
    })
    ...
    const fiberM = await prisma.product.create({
        data: {
            ...
            features: JSON.stringify(['Flatrate ins dt. Festnetz', 'Internet Flat']),
        }
    })
    ...
    const fiberXL = await prisma.product.create({
        data: {
            ...
            allowMagentaTV: true,
            hasMagentaTVBundle: true,
            features: JSON.stringify(['Flatrate ins dt. Festnetz', 'Internet Flat', 'SuperVectoring']),
        }
    })
    ...
    await prisma.product.create({
        data: {
            ...
            basePrice: 10.00,
            contractDuration: 24,
            features: JSON.stringify(['100+ Sender', 'HD', 'RTL+ Premium', 'MagentaTV App']),
        }
    })
    ...
    await prisma.product.create({
        data: {
            ...
            basePrice: 15.00,
            contractDuration: 1,
            features: JSON.stringify(['100+ Sender', 'HD', 'RTL+ Premium', 'Monatlich kündbar']),
        }
    })
    ...
    await prisma.product.create({
        data: {
            ...
            features: JSON.stringify(['Wi-Fi 6', 'Mesh master', 'SmartHome Hub']),
        }
    })
    ...
    await prisma.specialPrice.create({
        data: {
            ...
            priority: 10,
            products: {
                connect: [{ id: mobilM.id }, { id: mobilL.id }]
            },
            tiers: {
                create: [
                    { price: 29.95, fromMonth: 1, toMonth: 6 }
                ]
            }
        }
    })
    ...
    await prisma.specialPrice.create({
        data: {
            ...
            priority: 10,
            products: {
                connect: [{ id: fiberM.id }, { id: fiberXL.id }]
            },
            tiers: {
                create: [
                    { price: 19.95, fromMonth: 1, toMonth: 6 },
                    { price: 34.95, fromMonth: 7, toMonth: 12 },
                ]
            }
        }
    })
    ...
    await prisma.specialPrice.create({
        data: {
            ...
            priority: 20,
            products: {
                connect: [{ id: fiberM.id }]
            },
            tiers: {
                create: [
                    { price: 39.95, fromMonth: 1, toMonth: 12 }
                ]
            }
        }
    })
    ...
    await prisma.addon.create({
        data: {
            ...
            tiers: {
                create: [
                    { name: 'Standard mit Werbung', price: 4.99 },

...
// O(1) lookup map for streaming services to avoid O(N) array finds
const STREAMING_SERVICES_MAP = new Map(STREAMING_SERVICES.map(s => [
 s.id,
s,
]));
...
export function StreamingComparison({
	...
	const getPrice = useCallback((id: string) => {
		...
		return STREAMING_SERVICES_MAP.get(id)?.price || 0;
	}, [
		customPrices,
	]);
	...
	const toggleService = (groupId: string, id: string | null) => {
		setSelectedServices((prev) => {
			...
			const filtered = prev.filter((sId) => {
				const service = STREAMING_SERVICES_MAP.get(sId);
	...
	const coveredValue = useMemo(() => {
		return selectedServices.reduce((sum, currentServiceId) => {
			const currentService = STREAMING_SERVICES_MAP.get(currentServiceId);
			...
			const includedServiceIdForGroup = targetPlan.includedServiceIds.find(
				(serviceId) => {
					const incService = STREAMING_SERVICES_MAP.get(serviceId);
									...
									<div className="flex flex-col gap-6">
										...
										<div className="grid gap-5">
											{groupedServices.map((group, idx) => {
												// ⚡ Bolt Optimization: Pull repeated find() out of mapping to reduce O(N*M) to O(N)
												const selectedServiceIdForGroup = selectedServices.find((sId) =>
													STREAMING_SERVICES_MAP.get(sId)?.group === group.groupId,
												) || null;
												...
												return (
													<TierSelect
														...
														selectedId={selectedServiceIdForGroup}
														onSelect={(id) => toggleService(group.groupId, id)}
														customPrice={
															selectedServiceIdForGroup
																? customPrices[selectedServiceIdForGroup]
																: undefined
														}
															...
															{plan.includes.map((inc, i) => {
																const isGroupSelected =
																	...
																	selectedServices.some((sId) => {
																		const s = STREAMING_SERVICES_MAP.get(sId);

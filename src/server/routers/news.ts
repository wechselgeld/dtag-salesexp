...
async function getSessionContext(ctx: any) {
    ...
    return { odRegionId, locationId, teamId, isAdmin };
}

export const newsRouter = router({
    listActive: publicProcedure.query(async ({ ctx }) => {
        const { odRegionId, locationId, teamId, isAdmin } = await getSessionContext(ctx);

        const whereClause: any = { isActive: true };

        if (!isAdmin) {
            whereClause.OR = [
                { odRegionId: null, locationId: null, teamId: null }, // GLOBAL
            ];
            if (odRegionId) whereClause.OR.push({ odRegionId });
            if (locationId) whereClause.OR.push({ locationId });
            if (teamId) whereClause.OR.push({ teamId });
        }
    ...
    onAdd: publicProcedure.subscription(async function* ({ ctx, signal }) {
        const { odRegionId, locationId, teamId, isAdmin } = await getSessionContext(ctx);

        for await (const [news] of on(newsEmitter, 'add', { signal })) {
            if (isAdmin) {
                yield news;
                continue;
            }

...
export const availabilityRouter = router({
        ...
        .mutation(async ({ input }) => {
            ...
            try {
                ...
                if (!avcResponse.ok) {
                    ...
                    throw new Error('Telekom API availabilityCheck failed');
                }
                ...
                const tariffsPayload = {
                    ...
                    "selectedAttributes": [
                        { "name": "isSwitchingProvider", "value": false },
                        { "name": "isYoung", "value": false }
                    ]
                };
                ...
                for (let attempt = 0; attempt < maxRetries; attempt++) {
                    try {
                        ...
                        console.log(`[Tariffs] HTTP ${tariffsResponse.status} | tariffs: ${(data.tariffs || []).length} | error: ${data.error || false} | code: ${data.code || 'none'}`);
                ...
                throw new Error(error.message || 'Internal error check availability');
            }
        }),
    ...
    suggestAddress: publicProcedure
        .input(z.object({
            searchTerm: z.string(),
            searchTarget: z.enum(['placeAndZip', 'streetName', 'houseNumber']).optional(),
